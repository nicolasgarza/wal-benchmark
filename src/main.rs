...
fn main() {
    ...
    println!("---------------");
    println!(
        "testing different thread counts - BATCHING, size = {}",
        BATCH_SIZE
    );
    let config = ExperimentConfig {
        threads: vec![1, 4, 16, 64],
...
fn runner(config: ExperimentConfig, cores: &Vec<core_affinity::CoreId>) {
    for ((&n_threads, &writing), &iteration) in config
        .threads
        .iter()
        .zip(&config.to_write)
        .zip(&config.iterations)
    {
...
fn print_aggregates(times: Vec<f64>, threads: u64, iterations: u64, elapsed: f64) {
    ...
    println!(
        ...
        threads, iterations, avg_thread_time, writes_per_sec,
    );
}
